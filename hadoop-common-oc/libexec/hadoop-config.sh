...
AUDITOR_DIR=${HDFS_DIR:-"share/hadoop/auditor"}
AUDITOR_LIB_JARS_DIR=${HDFS_LIB_JARS_DIR:-"share/hadoop/auditor/lib"}
...
if [ -d "$HADOOP_PREFIX/$AUDITOR_LIB_JARS_DIR" ]; then
  CLASSPATH=${CLASSPATH}:$HADOOP_PREFIX/$AUDITOR_LIB_JARS_DIR'/*'
fi

CLASSPATH=${CLASSPATH}:$HADOOP_PREFIX/$AUDITOR_DIR'/*'
